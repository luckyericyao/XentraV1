...
export function OperatingFlow({ steps }: OperatingFlowProps) {
  return (
    <ol className="mt-10 border-y border-[#2A2D33]">
      {steps.map((step, index) => (
        <li
          key={step.title}
          className="scroll-rise grid gap-5 border-b border-[#2A2D33] py-5 last:border-b-0 transition duration-500 hover:bg-[#101214] md:grid-cols-[4rem_0.55fr_1fr] md:items-start md:py-6"
        >
